...
const decisionProfiles={aggressive:{label:'赌狗',action:'看涨 · 4.20U',confidence:76,mode:'连胜加码',reason:'短线、动量和主动买卖同向，当前两连胜，按策略加码。'},smart:{label:'超级AI',action:'看涨 · 2.00U',confidence:83,mode:'先判局势',reason:'ADX、EMA 和 MACD 判断为趋势局，再用流量与盘口确认方向。'},conservative:{label:'守财奴',action:'本轮不下注',confidence:62,mode:'捂紧钱袋',reason:'当前波动或价差偏高，即使方向看涨也不押。'}};
...
function liveDetail(key,m){
  ...
  const modelCaption=document.createElement('span');modelCaption.textContent='本局模型';
  const modelValue=document.createElement('strong');
  if(policy.aiConnectionId!==undefined) {
    modelValue.textContent=window.Warrior.battleModels?.label(policy)||'不使用 AI（本地规则）';
  ...
  const mode=document.createElement('span');mode.textContent=({NORMAL:'正常',ADD_ON:'加码',ALL_IN:'条件梭哈',WAIT:'等待信号'})[decision?.riskMode]||'等待信号';meta.append(round,mode);
  const headline=document.createElement('div');headline.className='decision-headline';
  const actionWrap=document.createElement('div'),actionLabel=document.createElement('small'),action=document.createElement('h3');actionLabel.textContent='AI 决策';
  action.textContent=!decision?'等待 AI 决策':decision.action==='BET'?`${policy.coin||'BTC'} · ${decision.direction==='UP'?'看涨':'看跌'} · ${Number(decision.stake ?? decision.stakeUsdt ?? 0).toFixed(2)}U`:decision.action==='REJECTED'?'本轮被风控拒绝':'本轮不下注';
  ...
  actionWrap.append(actionLabel,action);const confidence=document.createElement('strong'),confidenceValue=document.createElement('span');confidenceValue.dataset.noTranslate='';confidenceValue.textContent=decision?`${Number(decision.confidence).toFixed(0)}%`:'—';const confidenceLabel=document.createElement('small');confidenceLabel.textContent='置信度';confidence.append(confidenceValue,confidenceLabel);headline.append(actionWrap,confidence);
  const reason=document.createElement('p');reason.textContent=decision?.reason||'收到完整且新鲜的指标后，AI 可以选择看涨、看跌或不下注。';featured.append(meta,headline,reason);
  if(strategyProfiles[policy.strategy]&&window.WarriorStrategyCatalog.divinationStrategies.includes(policy.strategy)) {
    ...
    if(decision?.divination) {
      ...
      if(decision.engine?.mode==='deepseek')reason.textContent=decision.divination.interpretation||decision.reason;
  ...
  const metricsHeading=document.createElement('summary');metricsHeading.className='indicator-heading';const metricsTitle=document.createElement('strong');metricsTitle.textContent='本轮指标快照';metricsHeading.append(metricsTitle);if(!decision){const metricsState=document.createElement('span');metricsState.textContent='等待指标';metricsHeading.append(metricsState);}
  ...
  const rows=[
    ...
    ['RSI 14',Number.isFinite(source.rsi_14)?Number(source.rsi_14).toFixed(2):null],
    ['EMA 5 / 20',source.ema_5_20?`${Number(source.ema_5_20.ema5).toFixed(2)} / ${Number(source.ema_5_20.ema20).toFixed(2)}`:null],
    ['成交量倍率',Number.isFinite(source.volume_ratio)?`${Number(source.volume_ratio).toFixed(2)}×`:null],
    ['订单簿失衡',Number.isFinite(source.spot_order_book_imbalance)?`${(Number(source.spot_order_book_imbalance)*100).toFixed(2)}%`:null],
  ...
  if(!rows.length){const empty=document.createElement('p');empty.textContent='等待指标';grid.append(empty)}metrics.append(metricsHeading,grid);
  const policyNode=document.createElement('section');policyNode.className='decision-policy';
  [['决策变化',`${policy.decisionVariance??45}/100`],['手痒程度',`${policy.actionUrge??strategyProfiles[policy.strategy]?.actionUrge??50}/100`],['情绪波动',`${policy.emotionSensitivity??strategyProfiles[policy.strategy]?.emotionSensitivity??0}/100`],['单轮上限',`${policy.maxStakePct??100}%`],['条件梭哈',policy.allowAllIn?'允许':'关闭']].forEach(([label,value])=>{const item=document.createElement('div'),labelNode=document.createElement('span'),valueNode=document.createElement('strong');labelNode.textContent=label;valueNode.textContent=value;item.append(labelNode,valueNode);policyNode.append(item)});
  const note=document.createElement('p');note.className='dialog-note';note.textContent=decision?.engine?.mode==='deepseek'?'DeepSeek 决策 · 仅模拟下注':'本地模拟 AI · 未调用 DeepSeek';
  const policyDetails=document.createElement('details');policyDetails.className='detail-disclosure';const policyTitle=document.createElement('summary');policyTitle.textContent='策略参数';policyDetails.append(policyTitle,policyNode);
