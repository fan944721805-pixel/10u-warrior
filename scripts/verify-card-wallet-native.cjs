...
(async()=>{
 const server=http.createServer((req,res)=>res.end('<!doctype html><meta charset="utf-8"><button id="wallet-shortcut"></button><dialog id="wallet-dialog"><div id="wallet-content"></div></dialog>'));
 ...
 try{
  ...
  for(const headerName of ['set-cookie','Set-Cookie']){
   ...
   await page.addScriptTag({content:fs.readFileSync(path.join(root,'public/card-wallet.js'),'utf8')});
   await page.evaluate(()=>{window.__wallet=WarriorCardWallet.mount({dialog:document.querySelector('#wallet-dialog'),shortcut:document.querySelector('#wallet-shortcut'),t:key=>key,request:async(url,body)=>{const r=await __runtime.request(url,{method:body===undefined?'GET':'POST',...(body===undefined?{}:{body:JSON.stringify(body)})});const d=await r.json();if(!r.ok)throw Object.assign(Error(d.code),{code:d.code});return d;},openUrl:async()=>{}});});
   ...
   await page.evaluate(async()=>{await __wallet.open();await __wallet.service.signin();});
   assert.equal(await page.evaluate(()=>__wallet.service.snapshot().status),'awaiting_scan');
   await page.evaluate(async()=>{__confirmed=true;await __wallet.service.poll();});
   assert.equal(await page.evaluate(()=>__wallet.service.snapshot().status),'awaiting_scan','App confirmation alone is insufficient');
   await page.evaluate(()=>{__created=true;window.dispatchEvent(new Event('warrior-android-resume'));});
   await page.waitForFunction(()=>__wallet.service.snapshot().status==='connected');
   assert.ok(await page.evaluate(()=>__queriesWithCookie>0));
   assert.ok(await page.locator('[data-wallet-action=signout]').isVisible());
   const restored=await page.evaluate(async()=>{
    __wallet.service.close();__runtime.api.dispose();__runtime=WarriorMobileRuntime.create({autoStart:false});
